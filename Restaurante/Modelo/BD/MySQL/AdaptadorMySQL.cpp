...
#include "AdaptadorMySQL.hpp"

std::shared_ptr<AdaptadorMySQL> AdaptadorMySQL::obtenerInstancia() {
	static std::shared_ptr<AdaptadorMySQL> instanciaUnica = nullptr;

	if (instanciaUnica == nullptr) {
		instanciaUnica = std::make_shared<AdaptadorMySQL>();
	}
	return instanciaUnica;
}

AdaptadorMySQL::AdaptadorMySQL() {
	// Obtener singleton
...
void AdaptadorMySQL::mostrarMenu() {
...
std::vector<std::string> AdaptadorMySQL::mostrarHistorialVentas() {
...
Platillo AdaptadorMySQL::obtenerInfoPlatillo(int idPlatillo) {
...
void AdaptadorMySQL::registarVenta(float subtotal, float iva, float total) {
